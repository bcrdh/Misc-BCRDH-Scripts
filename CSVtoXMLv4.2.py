        ...
        if news: #if we are converting newspaper issue metadata
            for item in df.itertuples():
                ...
                if item.Publisher_Original is not None:
                    xmlString += '<originInfo><publisher>' + item.Publisher_Original + '</publisher></originInfo>'
                # CHANGED
                if item.Publisher_Location is not None:
                    xmlString += '<originInfo><place><placeTerm type="text">' + item.Publisher_Location + '</placeTerm></place></originInfo>'
                # CHANGED
